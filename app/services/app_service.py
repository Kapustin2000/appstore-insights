"""
App service for fetching app information and reviews from iTunes API.
"""
...
def fetch_app_info(app_id: str, country: str) -> Dict[str, Any]:
    ...
    try:
        r = requests.get(
            ...
            headers={"User-Agent": settings.itunes_user_agent},
            timeout=settings.itunes_timeout,
        )
        r.raise_for_status()
        js = r.json()
...
def fetch_reviews_paged(app_id: str, country: str, limit: int = 300, delay: float = 1.0) -> Dict[str, Any]:
    ...
    while len(items) < limit:
        ...
        try:
            r = requests.get(url, headers=headers, timeout=settings.itunes_timeout)
            ...
            has_next = any(
                (link.get("attributes", {}) or {}).get("rel") == "next"
                for link in links
    ...
    return {"status": "success", "items": items, "meta": {"pages_fetched": pages_fetched}}
